...
public class CategorysResource {
	...
	public void newCategory(@FormParam("categoryId") String categoryId,
			@FormParam("categoryName") String categoryName,
			@Context HttpServletResponse servletResponse) throws IOException {
		Category c = new Category(Integer.parseInt(categoryId), categoryName);
		CategoryStore.getStore().put(categoryId, c);

		URI uri = uriInfo.getAbsolutePathBuilder().path(categoryId.toString())
				.build();
